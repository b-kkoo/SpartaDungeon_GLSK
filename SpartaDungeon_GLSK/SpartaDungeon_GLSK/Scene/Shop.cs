                    ...
                    switch (keyInput)
                    {
                        ...
                        case ConsoleKey.D9:
                            ...
                            if (selectedIdx + potionTab < _sellingPotionList.Count)
                            {
                                ...
                                Console.Write("→");
                                if (GetQuantityInput(35, 7, 99, keyController, ref quantity))
                                {
                                    Console.SetCursorPosition(0, 4);
                                    for (int i = 0; i <= 25; i++) Console.WriteLine(new string(' ', Console.WindowWidth));
                                    Console.SetCursorPosition(0, 0);
                    ...
                    switch (keyInput)
                    {
                        ...
                        case ConsoleKey.D9:
                            ...
                            if (selectedIdx + potionTab < Program.playerData.invenPotion.Count)
                            {
                                ...
                                Console.SetCursorPosition(37, 7);
                                Console.Write("→");
                                if (GetQuantityInput(35, 7, Program.playerData.invenPotion[selectedIdx + potionTab].stack, keyController, ref quantity))
                                {
                                    Console.SetCursorPosition(0, 4);
                                    for (int i = 0; i <= 25; i++) Console.WriteLine(new string(' ', Console.WindowWidth));
                                    Console.SetCursorPosition(0, 0);
                    ...
                    switch (keyInput)
                    {
                        ...
                        case ConsoleKey.D9:
                            ...
                            if (selectedIdx + gearTab < _sellingGearList.Count)
                            {
                                ...
                                Console.SetCursorPosition(0, 0);
                                Console.SetCursorPosition(0, 4);
                                Console.WriteLine($"{gear.name}를 구매하시겠습니까?\n");
                                ...
                                while (loop3)
                                {
                                    ...
                                    {
                                        case ConsoleKey.Z:
                                            if (Program.playerData.Gold >= gear.Price)
                                            {
                                                //골드 차감
                                                Program.playerData.Gold -= gear.Price;

                                                //인벤토리에 아이템 추가
                                                Program.playerData.invenGear.Add(new WorldGear(_sellingGearList[selectedIdx + gearTab]));

                                                //구매 메시지
                                                Console.WriteLine($"{gear.name}를 구매하였습니다");
                                                Thread.Sleep(1000);
                                                keyController.GetUserInput(keyFilter, out cheatActivated);
                                            }
                                            else //골드 부족
        ...
        public static bool SellGear(out Scenes next, KeyController keyController)
        {
            ...
            while (loop)
            {
                ...
                for (int i = gearTab; i < gearTab + 9 && i < Program.playerData.invenGear.Count; i++)
                {
                    ...
                    int wearer = Program.playerData.invenGear[i].wearer;

                    Console.Write($" {i - gearTab + 1}. {gear.name}");
                    if (wearer >= 0)
                    {
                        Console.Write($"  ({Program.playerData.team[wearer].Name}이 착용 중)");
                    }
                    Console.WriteLine("");
                }
                ...
                while (loop2)
                {
                    ...
                    switch (keyInput)
                    {
                        ...
                        case ConsoleKey.D9:
                            ...
                            {
                                int wearer = Program.playerData.invenGear[selectedIdx + gearTab].wearer;
                                ...
                                Console.SetCursorPosition(0, 0);
                                Console.SetCursorPosition(0, 4);
                                Console.WriteLine($"{gear.name}를 판매하시겠습니까?\n");
                                Console.WriteLine("                                 (Z : 예,  X : 아니오)");
                                    ...
                                    switch (keyInput)
                                    {
                                        case ConsoleKey.Z:
                                            ...
                                            if (wearer >= 0)
                                            {
                                                ...
                                                Program.playerData.team[wearer].UnequipGear(wearer, gs);
                                            }
                                            //인벤토리에 아이템 제거
                                            Program.playerData.invenGear.RemoveAt(selectedIdx + gearTab);

                                            //구매 메시지
                                            Console.WriteLine($"{gear.name}를 판매하였습니다");
                                            Thread.Sleep(1000);
                                            keyController.GetUserInput(keyFilter, out cheatActivated);
